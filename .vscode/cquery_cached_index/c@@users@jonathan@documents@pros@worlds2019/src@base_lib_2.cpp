...
#define autoStraightP 380.0
#define autoStraightD 70.0
...
void baseControl(void * ignore){
  double prevErrorL = 0, prevErrorR = 0, prevErrorAngle = 0, prevError = 0;
	int count = 0;
  while(competition::is_autonomous()){
    ...
    motorTargetR = kP*errorR+kD*deltaErrorR;*/
		...
		double error = (targetL-leftPos+targetR-rightPos)/2;  //Cancels out turning (One positive, one negative)
		double deltaError = error-prevError;
		motorTargetL = kP*error+kD*deltaError;
    motorTargetR = kP*error+kD*deltaError;
		...
		if(count++ % 4 == 0) printf("%f\t%f\t%f\n",motorTargetL,motorTargetR,position.angle/PI*180);
...
void waitBase(double cutoff){
	double start = millis();
	while((fabs(targetAngle-position.angle) > ANGLE_LEEWAY/180*PI || fabs(targetL-BL.get_position())>DISTANCE_LEEWAY || fabs(targetR-BR.get_position())>DISTANCE_LEEWAY) && millis()-start < cutoff){
